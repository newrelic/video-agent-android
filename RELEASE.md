...
1. Add your test branch to workflow triggers:
...
```json
{
  "branches": ["master", "your-test-branch"]
}
```
...
echo "test" >> README.md && git add . && git commit -m "fix: test patch"
echo "test" >> README.md && git add . && git commit -m "feat: test minor"
echo "test" >> README.md && git add . && git commit -m 'feat!: test major'
git push origin local-test
```

3. Create PR to your test branch and observe the workflow.
...
**Cause:** Using `angular` preset instead of `conventionalcommits`
